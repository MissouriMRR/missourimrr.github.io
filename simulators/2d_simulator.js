...
var c = document.getElementById("simulation_canvas");
...
function GenerateMatch()
{
  ...
  alert("Match generated, click the Play button to begin!");
...
function Engine(CurrentFrame)
{
  ...
  // Done copying data from last frame
    ...
    for(var index = 0; index < DRONE_NUMBER; index++)
    {
      ...
      if(DataArray[CurrentFrame].Drones[index].OkToLeave == true && (NextRotateFrame - CurrentFrame) / FRAME_RATE > 12 && DataArray[CurrentFrame].GroundRobots[DataArray[CurrentFrame].Drones[index].TargetRobot].StillIn == true)
      {
        var TempX = 0;
        var TempY = 0;
        for(var index2 = 0; index2 < GROUND_ROBOT_NUMBER; index2++)
        {
          ...
          var TargetRot = 180;
          if(DataArray[CurrentFrame].GroundRobots[index2].StillIn == true && (Rot < TargetRot - 70 / 2 || Rot > TargetRot + 70 / 2))
          {
            var SafeTarget = true;
      ...
      var TargetX = DataArray[CurrentFrame].GroundRobots[DataArray[CurrentFrame].Drones[index].TargetRobot].X;
      var TargetY = DataArray[CurrentFrame].GroundRobots[DataArray[CurrentFrame].Drones[index].TargetRobot].Y;
      ...
      if(Math.pow(A, 2) + Math.pow(B, 2) <= Math.pow(GroundRobotPixelRadius, 2))
      {
        if(DataArray[CurrentFrame].GroundRobots[DataArray[CurrentFrame].Drones[index].TargetRobot].FramesToTurn <= 0)
          ...
          //alert(Rot);
          //alert(DataArray[CurrentFrame].GroundRobots[DataArray[CurrentFrame].Drones[index].TargetRobot].X + "\n");
          var TargetRot = 0;
          ...
          if(Rot < TargetRot - 70 / 2 || Rot > TargetRot + 70 / 2 && DataArray[CurrentFrame].GroundRobots[DataArray[CurrentFrame].Drones[index].TargetRobot].StillIn == true)
		      {
...
{
  var timeInSeconds = (CurrentFrame / FRAME_RATE);
  var timeInMinutes = (timeInSeconds / 60);
  document.getElementById("ShowPosition").innerHTML = timeInMinutes.toFixed(2);
